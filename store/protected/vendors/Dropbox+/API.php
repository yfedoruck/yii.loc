...
class Dropbox_API {
     ...
     * @return bool 
     */
    public function putFile($path, $file, $action = "move", $root = null) {
     ...
     * @return stdclass 
     */
    public function copy($from, $to, $from_copy_ref = null, $root = null) {

        if (is_null($root)) $root = $this->root;
        if(!$from_copy_ref) $response = $this->oauth->fetch($this->api_url . 'fileops/copy', array('from_path' => $from, 'to_path' => $to, 'root' => $root));
        else $response = $this->oauth->fetch($this->api_url . 'fileops/copy', array('from_copy_ref' => $from_copy_ref, 'to_path' => $to, 'root' => $root));
        return json_decode($response['body'],true);

    }

    public function copyRef($path,$root = null)   {
        if (is_null($root)) $root = $this->root;
        echo $path;
        $response = $this->oauth->fetch($this->api_url."copy_ref/$path",array("path" =>$path, "root"=>$root));
        return json_decode($response['body'],true);
    }
    /**
    ...
    public function share($url, $root = null) {
        if (is_null($root)) $root = $this->root;
		$url = str_replace(array('%2F','~'), array('/','%7E'), rawurlencode($url));
    ...
    public function revisions($url, $root = null) {
        if (is_null($root)) $root = $this->root;
        $url = str_replace(array('%2F','~'), array('/','%7E'), rawurlencode($url));
        $response = $this->oauth->fetch($this->api_url.  'revisions/'. $root . '/' . ltrim($url, '/'));
        return json_decode($response['body'],true);
    }

    public function restore($url, $revision, $root = null)   {
        if (is_null($root)) $root = $this->root;
        $url = str_replace(array('%2F','~'), array('/','%7E'), rawurlencode($url));
        $response = $this->oauth->fetch($this->api_url.  'restore/'. $root . '/' . ltrim($url, '/'),array("rev"=>$revision));
        return json_decode($response['body'],true);
    }
	
	public function media($url, $root = null) {
        if (is_null($root)) $root = $this->root;
        $url = str_replace(array('%2F','~'), array('/','%7E'), rawurlencode($url));
        $response = $this->oauth->fetch($this->api_url.  'media/'. $root . '/' . ltrim($url, '/'));
        return json_decode($response['body'],true);
        

    }
    
    public function getDownloadUrl($url, $root = null)
    {
        if (is_null($root)) $root = $this->root;
        $response = $this->media($url, $root);
		// ?dl=1 - download immediately
		if($response['url'])
			return $response['url'].'?dl=1';
		return false;
	}
}
